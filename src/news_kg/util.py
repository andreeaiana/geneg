...
NAMESPACE_RENEWRS_BASE = utils.get_config('geneg.namespace.base')
NAMESPACE_RENEWRS_PROPERTY = utils.get_config('geneg.namespace.property')
NAMESPACE_RENEWRS_RESOURCE = utils.get_config('geneg.namespace.resource')
...
def id2geneg_resource(node_id: str) -> str:
    return NAMESPACE_RENEWRS_RESOURCE + node_id


def geneg_resource2id(geneg_resource: str) -> str:
    return geneg_resource[len(NAMESPACE_RENEWRS_RESOURCE):]


def label2geneg_resource(label: str) -> str:
    return NAMESPACE_RENEWRS_RESOURCE + label
...
def is_geneg_property(item: str) -> bool:
    return item.startswith(NAMESPACE_RENEWRS_PROPERTY)


def label2geneg_property(label: str) -> str:
    return NAMESPACE_RENEWRS_PROPERTY + label


def geneg_property2label(geneg_property: str) -> str:
    return geneg_property[len(NAMESPACE_RENEWRS_PROPERTY):]
