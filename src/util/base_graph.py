...
class BaseGraph:
    """ A simple directed graph with nodes and edges. """
    def __init__(self, graph: nx.MultiDiGraph) -> None:
        self.graph = graph

    def copy(self):
        new_self = self.__class__(copy.deepcopy(self.graph))
        return new_self
    ...
    def depths(self, source_node) -> dict:
    ...
    def _update_attr(self, node, attr, val):
    ...
    def _remove_attr_val(self, node, attr, val):
    ...
    def _set_edge_attr(self, edge, attr, val):
        self.graph[edge[0]][edge[1]][attr] = val

    def _reset_edge_attr(self, edge, attr):
    ...
    def contracted_nodes(self, u, v, attr2update: list, self_loops=True):
